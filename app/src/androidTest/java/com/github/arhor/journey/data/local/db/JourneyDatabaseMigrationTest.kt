...
class JourneyDatabaseMigrationTest {
    ...
    fun migrate1To2_should_remap_legacy_resource_ids_and_spawn_suffixes() {
        ...
        val migrated = Room.databaseBuilder(
            ...
            JourneyDatabase::class.java,
            dbName,
        ).addMigrations(*JourneyDatabase.Companion.MIGRATIONS)
            .build()
    ...
    fun migrate3To4_should_drop_persisted_definitions_and_reset_watchtower_state() = runBlocking {
        ...
        val migrated = migrationHelper.runMigrationsAndValidate(
            ...
            4,
            true,
            JourneyDatabase.Companion.MIGRATION_3_4,
        )
        ...
        try {
            ...
            val repository = DeterministicWatchtowerRepository(
                dao = database.watchtowerStateDao(),
    ...
    fun migrate2To3_should_create_watchtower_tables_and_index() {
        ...
        val migrated = migrationHelper.runMigrationsAndValidate(
            ...
            3,
            true,
            JourneyDatabase.Companion.MIGRATION_2_3,
        )
    ...
    fun migrate4To5_should_drop_poi_tables_and_preserve_other_data() {
        ...
        val migrated = migrationHelper.runMigrationsAndValidate(
            ...
            5,
            true,
            JourneyDatabase.Companion.MIGRATION_4_5,
        )
    ...
    private fun SupportSQLiteDatabase.foreignKeyCount(tableName: String): Int =
    ...
    private fun SupportSQLiteDatabase.intScalar(sql: String): Int =
        query(sql).use { cursor ->
            cursor.moveToFirst()
            cursor.getInt(0)
        }

    private fun SupportSQLiteDatabase.stringScalar(sql: String): String =
        query(sql).use { cursor ->
            cursor.moveToFirst()
            cursor.getString(0)
        }
