...
const val RESOURCE_SPAWN_GENERATOR_VERSION = 1
...
@Singleton
class DeterministicResourceSpawnGenerator @Inject constructor() {

    suspend fun activeSpawns(query: ResourceSpawnQuery): List<ResourceSpawn> {
    ...
    private fun generateSpawn(
        ...
        val resourceType = ResourceType.generationOrderedEntries[
            stableIndex(
                seed = "type:$RESOURCE_SPAWN_GENERATOR_VERSION:$daySeed:$cellX:$cellY:$slot",
                size = ResourceType.generationOrderedEntries.size,
            )
        ]
        val seedPrefix =
            "$RESOURCE_SPAWN_GENERATOR_VERSION:$daySeed:$cellX:$cellY:$slot:${resourceType.generationTypeId}"
    ...
    private fun buildSpawnId(
        ...
        "resource-spawn:v$RESOURCE_SPAWN_GENERATOR_VERSION:${day.toEpochDay()}:$cellX:$cellY:$slot:$resourceTypeId"
    ...
    private fun parseSpawnId(spawnId: String): SpawnDescriptor? {
        ...
        if (version != RESOURCE_SPAWN_GENERATOR_VERSION) {
            return null
        }
    ...
    private fun ResourceSpawnQuery.toSearchBounds(): GeoBounds? {
        ...
        val centerBounds = center?.let { location ->
            location.toBounds(radiusMeters ?: 0.0)
        }
