...
## [method setup] combat with a [CombatArena].
...
## Optional arena to start immediately when this node enters the scene tree.
@export var initial_arena: PackedScene
...
func _ready() -> void:
	...
	if initial_arena:
		setup.call_deferred(initial_arena)
...
## Reserved hook for post-combat messaging in the combat-only app.
func _display_combat_results_dialog(_is_player_victory: bool) -> void:
	return
